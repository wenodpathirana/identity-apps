...
export const ConsumerUserProfile: FunctionComponent<ConsumerUserProfilePropsInterface> = (
     ...
     */
    const onCountryRefChange = useCallback(element => {
        if (element !== null) {
    ...
    const resolveAccountName = (user: ProfileInfoInterface) => {
        if (user.name && (user.name.givenName || user.name.familyName)) {
            const givenName = isEmpty(user.name.givenName) ? "" : user.name.givenName + " ";
            const familyName = isEmpty(user.name.familyName) ? "" : user.name.familyName;
    ...
    const handleSubmit = (values: Map<string, string | string[]>): void => {
        ...
        const booleanAttributes: Set<string> = new Set(profileSchema
            .map(schema => (schema.type.toUpperCase() === "BOOLEAN") ? schema.name : null));
        ...
        if (schema.type.toUpperCase() === "BOOLEAN") {
            ...
            return (
                <Field
                    ...
                    maxLength={ schema.name.toLowerCase().includes("url") ? -1 : 30 }
                    ref={ (element: HTMLElement) => assignReferenceToField(element, schema.name, key) }
                />
            );
        }
